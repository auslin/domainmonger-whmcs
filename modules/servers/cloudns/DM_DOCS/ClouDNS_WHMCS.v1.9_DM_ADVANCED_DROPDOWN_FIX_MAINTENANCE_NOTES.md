...
Switch Domain works on Mail Forwarding
Free SSL page loads
...
The Advanced dropdown was fixed after Menu Consistency Pass 1.

Changed behavior:

```text
Advanced dropdown opens normally
Menu no longer shows unnecessary scroll/arrow controls
Deactivate moved into Advanced at the bottom
Deactivate is separated from normal Advanced tools by a divider
```

Implementation details:

- Replaced menu `overflow-x: auto` with `overflow: visible`.
- Allowed top menu wrapping instead of scrollbar controls.
- Increased Advanced dropdown z-index.
- Moved Deactivate from the top-level menu into the Advanced dropdown.
- Added `.cloudns-advanced-divider` and `.cloudns-advanced-danger` styles.
- Mobile menu now lists `Advanced: Deactivate` under the Advanced section.

Existing parked-zone fixes and the Records count indicator remain in place.
