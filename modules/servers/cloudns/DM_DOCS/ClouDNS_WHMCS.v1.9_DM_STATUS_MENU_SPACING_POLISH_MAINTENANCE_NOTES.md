...
## Follow-up: Move Check Zone File to Status Page

The remaining external zone check link was moved out of the Advanced menu and onto the Status page.

Changed behavior:

```text
Top-level Status menu         -> unchanged
Advanced Check Zone File      -> not shown
Advanced Check on dns.computer -> not shown
Status page action row        -> Check Zone File + Request New Update
```

Implementation details:

- Removed the external check link from both desktop and mobile Advanced menus.
- Added `Check Zone File` as an external action button on `templates/update-status.tpl`.
- Added flex/gap styling to the Status page action row so both buttons align cleanly.
