...
## Export Dropdown Left Alignment Fix

The Export Zone File format dropdown text was changed from right-aligned to left-aligned.

Changed behavior:

```text
Visible "Format" text -> remains removed
Format dropdown       -> text is now left-justified
Format dropdown       -> text remains vertically centered with 34px control height
```

Implementation details:

- Updated `#cloudns-export-format` styling from:
  - `text-align: right`
  - `text-align-last: right`
- To:
  - `text-align: left`
  - `text-align-last: left`
- Kept the screen-reader-only Format label for accessibility.
- Kept button height consistency, DNSSEC blue content, right-justified compact icons, menu, Advanced dropdown, and parked-zone fixes in place.

## Main Menu Polish Pass

The main menu and Advanced menu labels/grouping were cleaned up.

Changed behavior:

```text
DNS Statistics -> Statistics
Zone Import    -> Import Zone File
Advanced menu  -> grouped with separators
Advanced pages -> active dropdown item highlight
Mobile menu    -> Advanced header with cleaner labels
```

Implementation details:

- Shortened the main Statistics label.
- Renamed Import label to `Import Zone File`.
- Added grouped separators inside the desktop Advanced dropdown:
  - DNS settings
  - Zone tools/checks
  - Export/import
  - Free SSL
  - Deactivate
- Added `.cloudns-advanced-active` styling for the active Advanced dropdown item.
- Removed repetitive `Advanced:` prefixes from the mobile menu and added a mobile `Advanced` dropdown header.
- Kept all routes/actions unchanged.
- Kept export dropdown left alignment, button height consistency, DNSSEC blue content, right-justified compact icons, menu behavior, Advanced dropdown behavior, and parked-zone fixes in place.

## Status and Advanced Menu Cleanup Pass

Status wording, Zone Transfers capitalization, duplicate Advanced link, and DNSSEC blue box spacing were cleaned up.

Changed behavior:

```text
Status page heading       -> Status for <domain>
Zone Transfers heading    -> Zone Transfers - <domain>
Advanced Check Zone File  -> removed because it duplicated top-level Status
DNSSEC blue info box      -> spacing/style matched closer to Zone Transfers notice box
```

Implementation details:

- Updated `templates/update-status.tpl` heading from `Servers update info` to `Status`.
- Updated `templates/zone-transfers.tpl` heading capitalization.
- Removed the duplicate `Check Zone File` entry from both desktop and mobile Advanced menus.
- Kept top-level `Status` as the canonical link to `customAction=update-status`.
- Updated `.cloudns-dnssec-panel .notification` to match the Zone Transfers blue notice styling more closely.
- Hid DNSSEC `br.clear` spacing inside the DNSSEC panel to make spacing more consistent.
- Kept Main Menu Polish, export dropdown left alignment, button height consistency, DNSSEC blue content, icon alignment, and parked-zone fixes in place.

## UI Consistency Pass 1

Standardized visible body/page headings across DNS Records, Status, SOA, DNSSEC, SSL, Zone Transfers, Import Zone File, and Export Zone File pages. Status/detail text now appears below the page title instead of replacing it. Capitalized selected generated page titles for consistency.


## DM Table Header Vertical Alignment Pass
- Vertically centered DNS Records table header text to align with the `+Add` button text.
- Vertically centered Mail Forwards table header text to align with the `+Add` button text.
- Kept the action column button styling and table behavior unchanged.
## DM Zones List Label Polish

- Changed the WHMCS Overview sidebar label from `Zones list` to `Zones List`.
- No module DNS/API behavior changed.
