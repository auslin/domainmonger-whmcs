...
Class Cloudns_Records {
	...
	/**
	 * Enables/returns the Dynamic URL for an A/AAAA DNS record.
	 *
	 * @param string $zone
	 * @param int|string $record_id
	 * @return array
	 */
	public function getDynamicUrl ($zone, $record_id) {
		$request = array('domain-name'=>$zone, 'record-id'=>$record_id);
		return $this->core->Api->call('dns/get-dynamic-url.json', $request);
	}

	/**
	 * Creates a standalone ClouDNS monitoring check.
	 *
	 * @param array $settings
	 * @return array
	 */
	public function createMonitoringCheck ($settings) {
		return $this->core->Api->call('monitoring/create.json', $settings);
	}
}
