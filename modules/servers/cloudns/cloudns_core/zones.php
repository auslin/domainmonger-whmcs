...
class Cloudns_Zones {
	...
	public function addMaster ($zone, $option, $servers, $zoneType = '', $templateZone = '') {
		...
		if (($zoneType !== 'masterZoneType' && empty($servers)) || $option == 2) {
			$ns = array('');
		} elseif (($option == 1 || $option == 3) && !empty($servers)) {
			// DomainMonger Patch 1590: include automatic product
			// nameservers during registration, including template zones.
			$ns = $servers;
		} 
	...
	/**
	 * Add a parked DNS zone.
	 *
	 * @param string $zone
	 * @return array
	 */
	public function addParked($zone) {
		$request = array('domain-name'=>$zone, 'zone-type'=>'parked');
	...
	/**
	 * Gets the per-server update status report for a zone.
	 * @param string $zone
	 * @return array
	 */
	public function getUpdateStatusReport ($zone) {
		$request = array('domain-name'=>$zone);

		return $this->core->Api->call('dns/update-status.json', $request);
	}
