...
<form action="clientarea.php?action=productdetails&id={$serviceid}&customAction=add-forward&zone={$zone}" method="post" id="recordsForm" class="recordsForm cloudns-forward-form cloudns-body-panel cloudns-form-panel cloudns-forward-form-panel">
	...
	<div class="clear"></div>
	<span class="spanHost"><input type="text" id="addRecordHost" name="addEmailForward" class="form-control" value="{if isset($settings.source)}{$settings.source}{/if}" autocapitalize="off" spellcheck="false" />@{$zone}</span><br class="dTitle">

	<br class="clear" />
	<div class="recordContainter">
		<div class="inputTitle">Points to:</div>
		<input type="email" id="addRecordRecord" name="addEmailForwardTo" value="{if isset($settings.destination)}{$settings.destination}{/if}" class="pointsTo form-control" autocapitalize="off" spellcheck="false" /> <br /><br />
	</div>
