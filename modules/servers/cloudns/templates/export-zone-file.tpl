...
.cloudns-export-actions .btn.cloudns-export-action-button,
.cloudns-export-actions button.btn.cloudns-export-action-button,
.cloudns-export-actions a.btn.cloudns-export-action-button,
.cloudns-export-actions .btn.cloudns-switch-style-button,
.cloudns-export-actions button.btn.cloudns-switch-style-button,
.cloudns-export-actions a.btn.cloudns-switch-style-button {
	display: inline-flex !important;
	align-items: center !important;
	justify-content: center !important;
...
.cloudns-export-actions .btn.cloudns-export-action-button:hover,
.cloudns-export-actions .btn.cloudns-export-action-button:focus,
.cloudns-export-actions button.btn.cloudns-export-action-button:hover,
.cloudns-export-actions button.btn.cloudns-export-action-button:focus,
.cloudns-export-actions a.btn.cloudns-export-action-button:hover,
.cloudns-export-actions a.btn.cloudns-export-action-button:focus,
.cloudns-export-actions .btn.cloudns-switch-style-button:hover,
.cloudns-export-actions .btn.cloudns-switch-style-button:focus,
.cloudns-export-actions button.btn.cloudns-switch-style-button:hover,
.cloudns-export-actions button.btn.cloudns-switch-style-button:focus,
.cloudns-export-actions a.btn.cloudns-switch-style-button:hover,
.cloudns-export-actions a.btn.cloudns-switch-style-button:focus {
	background: #f05f12 !important;
	background-color: #f05f12 !important;
	background-image: none !important;
	border-color: #f05f12 !important;
...
.cloudns-export-actions .btn.cloudns-export-action-button:active,
.cloudns-export-actions button.btn.cloudns-export-action-button:active,
.cloudns-export-actions a.btn.cloudns-export-action-button:active,
.cloudns-export-actions .btn.cloudns-switch-style-button:active,
.cloudns-export-actions button.btn.cloudns-switch-style-button:active,
.cloudns-export-actions a.btn.cloudns-switch-style-button:active {
	background: #dd520c !important;
	background-color: #dd520c !important;
	background-image: none !important;
	border-color: #dd520c !important;
...
/* DM page-by-page fix pass 1: export output grows to max 30 lines. */
.cloudns-export-textarea {
	min-height: 0 !important;
	height: auto;
	max-height: calc(30 * 1.45em + 22px) !important;
	line-height: 1.45 !important;
...
<div class="cloudns-export-panel">
	<div class="cloudns-export-help">
		Choose Export Format, then View or Download
	</div>

	<form method="get" action="clientarea.php" class="cloudns-export-actions">
		<input type="hidden" name="action" value="productdetails">
		<input type="hidden" name="id" value="{$serviceid}">
		<input type="hidden" name="customAction" value="export-zone-file">
		<input type="hidden" name="zone" value="{$zone|@htmlspecialchars}">
		<label for="cloudns-export-format" class="cloudns-sr-only">Format</label>
		<select name="format" id="cloudns-export-format" class="form-control">
			{foreach from=$exportFormats key=formatKey item=formatLabel}
				<option value="{$formatKey}"{if $exportFormat == $formatKey} selected="selected"{/if}>{$formatLabel|@htmlspecialchars}</option>
			{/foreach}
		</select>
		<button type="submit" class="btn cloudns-export-action-button cloudns-switch-style-button cloudns-btn-primary">View</button>
		<a class="btn cloudns-export-action-button cloudns-switch-style-button cloudns-btn-primary" href="clientarea.php?action=productdetails&id={$serviceid}&customAction=export-zone-file&zone={$zone}&format={$exportFormat}&download=1">Download</a>
	</form>

	{if isset($exportResponse.status) && $exportResponse.status == 'error'}
		<div class="cloudns-export-error">
			{if isset($exportResponse.description)}{$exportResponse.description|@htmlspecialchars}{else}Export failed.{/if}
		</div>
	{/if}

	<textarea class="form-control cloudns-export-textarea" readonly rows="8">{$exportZoneFile|@htmlspecialchars}</textarea>
</div>


<script>
(function () {
	var textarea = document.querySelector('.cloudns-export-textarea');
	if (!textarea) {
		return;
	}

	var lineHeight = parseFloat(window.getComputedStyle(textarea).lineHeight) || 18;
	var verticalPadding = 22;
	var lines = textarea.value ? textarea.value.split(/\r\n|\r|\n/).length : 1;
	var visibleLines = Math.max(8, Math.min(30, lines));
	var maxHeight = (lineHeight * 30) + verticalPadding;
	var targetHeight = (lineHeight * visibleLines) + verticalPadding;

	textarea.rows = visibleLines;
	textarea.style.height = targetHeight + 'px';
	textarea.style.maxHeight = maxHeight + 'px';
	textarea.style.overflowY = lines > 30 ? 'auto' : 'hidden';
})();
</script>
