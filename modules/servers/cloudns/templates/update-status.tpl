...
.cloudns-updated-panel h3 strong {
	font-weight: 700;
}
...
.cloudns-updated-list {
	margin: 0;
	padding: 0;
	list-style: none;
}

.cloudns-updated-server {
	display: flex;
	align-items: center;
...
.cloudns-updated-server:last-child {
	border-bottom: 0;
}

.cloudns-updated-server-details {
	min-width: 0;
	word-break: break-word;
}

.cloudns-updated-server-name {
	display: block;
	margin-bottom: 5px;
	font-weight: 700;
	color: #333333;
...
.cloudns-updated-status-ok {
	color: #2ecc71;
}

.cloudns-updated-status-pending {
	color: #f7941d;
}

.cloudns-updated-status-error {
	color: #a94442;
}
...
<div class="cloudns-updated-panel">
	<div class="cloudns-updated-summary">
		{if $updated === true || $updated == 1 || $updated == '1' || $updated == 'true'}
			<img src="{$path}statusok.gif" title="Updated" /> Your DNS zone is <strong>up to date</strong> on all servers.
		{else}
			<img src="{$path}loadingsml.gif" title="Not yet updated" /> Your DNS zone is still <strong>in update process</strong> and all changes will be deployed soon.
		{/if}
	</div>

	{if isset($updateReport) && is_array($updateReport) && !(isset($updateReport.status) && ($updateReport.status == 'error' || $updateReport.status == 'Failed'))}
		<ul class="cloudns-updated-list">
			{foreach from=$updateReport item=server key=serverKey}
				{if is_array($server)}
					{assign var="serverName" value=""}
					{assign var="serverIpv4" value=""}
					{assign var="serverIpv6" value=""}
					{assign var="serverUpdated" value=""}

					{foreach from=$server key=fieldKey item=fieldValue}
						{assign var="fieldKeyLower" value=$fieldKey|lower}
						{if $fieldKeyLower == 'server' || $fieldKeyLower == 'server_name' || $fieldKeyLower == 'server-name' || $fieldKeyLower == 'name' || $fieldKeyLower == 'location'}
							{assign var="serverName" value=$fieldValue}
						{elseif $fieldKeyLower == 'ipv4' || $fieldKeyLower == 'ip4' || $fieldKeyLower == 'ip' || $fieldKeyLower == 'address'}
							{assign var="serverIpv4" value=$fieldValue}
						{elseif $fieldKeyLower == 'ipv6' || $fieldKeyLower == 'ip6'}
							{assign var="serverIpv6" value=$fieldValue}
						{elseif $fieldKeyLower == 'updated' || $fieldKeyLower == 'status' || $fieldKeyLower == 'is_updated' || $fieldKeyLower == 'is-updated'}
							{assign var="serverUpdated" value=$fieldValue}
						{/if}
					{/foreach}

					{if $serverName == ''}
						{assign var="serverName" value=$serverKey}
					{/if}

					<li class="cloudns-updated-server">
						<div class="cloudns-updated-server-details">
							<span class="cloudns-updated-server-name">{$serverName|@htmlspecialchars}</span>
							<span class="cloudns-updated-server-meta">
								{if $serverIpv4 != ''}{$serverIpv4|@htmlspecialchars}{/if}
								{if $serverIpv4 != '' && $serverIpv6 != ''}<br>{/if}
								{if $serverIpv6 != ''}{$serverIpv6|@htmlspecialchars}{/if}
								{if $serverIpv4 == '' && $serverIpv6 == ''}
									{foreach from=$server key=fieldKey item=fieldValue}
										{if !is_array($fieldValue)}
											{$fieldKey|@htmlspecialchars}: {$fieldValue|@htmlspecialchars}<br>
										{/if}
									{/foreach}
								{/if}
							</span>
						</div>
						<div class="cloudns-updated-status {if $serverUpdated === true || $serverUpdated == 1 || $serverUpdated == '1' || $serverUpdated == 'true' || $serverUpdated == 'updated'}cloudns-updated-status-ok{elseif $serverUpdated === false || $serverUpdated == 0 || $serverUpdated == '0' || $serverUpdated == 'false'}cloudns-updated-status-pending{else}cloudns-updated-status-error{/if}">
							{if $serverUpdated === true || $serverUpdated == 1 || $serverUpdated == '1' || $serverUpdated == 'true' || $serverUpdated == 'updated'}&#10003;{elseif $serverUpdated === false || $serverUpdated == 0 || $serverUpdated == '0' || $serverUpdated == 'false'}&#8635;{else}!{/if}
						</div>
					</li>
				{/if}
			{/foreach}
		</ul>
	{elseif isset($updateReport) && is_array($updateReport) && isset($updateReport.statusDescription)}
		<div class="cloudns-updated-empty">{$updateReport.statusDescription|@htmlspecialchars}</div>
	{else}
		<div class="cloudns-updated-empty">No server update report was returned by the API.</div>
	{/if}

	<div class="cloudns-updated-actions">
		<a href="https://www.dns.computer/check/{$zone|@rawurlencode}" target="_blank" rel="noopener noreferrer" class="btn cloudns-switch-style-button cloudns-btn-secondary">Check Zone File</a>
		<a href="clientarea.php?action=productdetails&id={$serviceid}&customAction=update&zone={$zone}" class="btn cloudns-switch-style-button cloudns-btn-primary">Request New Update</a>
	</div>
</div>
