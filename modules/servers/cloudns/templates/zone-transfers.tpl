...
.cloudns-zone-transfer-response.cloudns-zone-transfer-response-error {
	border-color: #ebccd1;
	background: #f2dede;
	color: #a94442;
}
...
.cloudns-zone-transfer-add .cloudns-zone-transfer-save-wrap {
	grid-column: 1 / -1;
	text-align: center;
}
...
.cloudns-zone-transfer-button:hover,
.cloudns-zone-transfer-button:focus {
	text-decoration: none;
	outline: none;
}

.cloudns-zone-transfer-save {
	min-width: 88px;
}

.cloudns-zone-transfer-heading {
	font-size: 16px;
	font-weight: 700;
...
.cloudns-zone-transfer-row form {
	margin: 0;
}

.cloudns-zone-transfer-empty {
	padding: 18px 10px;
	text-align: center;
	font-style: italic;
}
...
/* DM page-by-page fix pass 1: compact Zone Transfers add row. */
.cloudns-zone-transfer-add {
	display: flex !important;
	align-items: center !important;
	justify-content: flex-start !important;
...
.cloudns-zone-transfer-add input.form-control,
.cloudns-zone-transfer-add #zone-transfer-ip {
...
@media only screen and (max-width: 650px) {
	...
	.cloudns-zone-transfer-add input.form-control,
	.cloudns-zone-transfer-add #zone-transfer-ip,
	.cloudns-zone-transfer-add .cloudns-zone-transfer-save,
	.cloudns-zone-transfer-add .cloudns-zone-transfer-save-wrap {
		width: 100% !important;
...
{if isset($response) && is_array($response) && (isset($response.description) || isset($response.statusDescription))}
	<div class="cloudns-zone-transfer-response {if isset($response.status) && ($response.status == 'error' || $response.status == 'Failed')}cloudns-zone-transfer-response-error{/if}">
		{if isset($response.description)}{$response.description|@htmlspecialchars}{else}{$response.statusDescription|@htmlspecialchars}{/if}
	</div>
{/if}

{if isset($listResponse) && is_array($listResponse) && (isset($listResponse.description) || isset($listResponse.statusDescription))}
	<div class="cloudns-zone-transfer-response cloudns-zone-transfer-response-error">
		{if isset($listResponse.description)}{$listResponse.description|@htmlspecialchars}{else}{$listResponse.statusDescription|@htmlspecialchars}{/if}
	</div>
{/if}

<div class="cloudns-zone-transfers-panel">
	<div class="cloudns-zone-transfer-notice">
		You can allow IPv4/IPv6 sub networks for zone transfers. There won't be notifications for sub networks and you must specify at least one full IP address to be notified for changes.
	</div>

	<form method="post" class="cloudns-zone-transfer-add" action="clientarea.php?action=productdetails&id={$serviceid}&customAction=zone-transfers-add&zone={$zone}">
		<label for="zone-transfer-ip">Add Slave Server IP:</label>
		<input type="text" name="ip" id="zone-transfer-ip" class="form-control" value="" autocomplete="off">
		<div class="cloudns-zone-transfer-save-wrap">
			<button type="submit" class="btn cloudns-zone-transfer-button cloudns-zone-transfer-save cloudns-switch-style-button cloudns-btn-primary">Save</button>
		</div>
	</form>

	<div class="cloudns-zone-transfer-heading">Slave Server IPs:</div>
	<div class="cloudns-zone-transfer-list">
		{if empty($zoneTransfers)}
			<div class="cloudns-zone-transfer-empty">You have not allowed IP addresses for AXFR's.</div>
		{else}
			{foreach from=$zoneTransfers item=transfer}
				<div class="cloudns-zone-transfer-row">
					<div>{$transfer.ip|@htmlspecialchars}</div>
					<form method="post" action="clientarea.php?action=productdetails&id={$serviceid}&customAction=zone-transfers-delete&zone={$zone}" onsubmit="return confirm('Delete this allowed IP address?');">
						<input type="hidden" name="id" value="{$transfer.id|@htmlspecialchars}">
						<button type="submit" class="btn btn-danger cloudns-zone-transfer-button cloudns-zone-transfer-delete cloudns-btn-danger">Delete</button>
					</form>
				</div>
			{/foreach}
		{/if}
