...
{literal}
<style id="dm-checkout-page-header-1442">
#order-standard_cart.dm-checkout-page > .dm-checkout-page-header {
...
#order-standard_cart.dm-checkout-page > .dm-checkout-page-header .dm-checkout-page-header-text {
    width: 100%;
    margin: 0;
    padding: 0;
    text-align: left !important;
}
#order-standard_cart.dm-checkout-page > .dm-checkout-page-header h1 {
    margin: 0 !important;
    padding: 0 !important;
    color: #fff !important;
...
#order-standard_cart.dm-checkout-page > .dm-checkout-page-header p {
...
/* Patch 1444: replace the inherited zero-height divider geometry on the
   two checkout headings so it cannot reserve a large blank strip below text. */
#order-standard_cart.dm-checkout-page .dm-checkout-account-heading,
#order-standard_cart.dm-checkout-page .dm-checkout-payment-heading {
...
#order-standard_cart.dm-checkout-page .dm-checkout-account-heading > span,
#order-standard_cart.dm-checkout-page .dm-checkout-payment-heading > span {
    display: block !important;
...
/* Patch 1445: keep the account-choice cards inset from their background panel. */
#order-standard_cart.dm-checkout-page #containerExistingAccountSelect.account-select-container {
    box-sizing: border-box !important;
    margin: 0 !important;
...
#order-standard_cart.dm-checkout-page #containerExistingAccountSelect > [class*="col-"] {
    padding-left: 0 !important;
    padding-right: 0 !important;
}
#order-standard_cart.dm-checkout-page #containerExistingAccountSelect .account {
    border-color: #d8e0e8 !important;
}
#order-standard_cart.dm-checkout-page #containerExistingAccountSelect > .col-sm-12 > .account {
...
<div id="order-standard_cart" class="dm-checkout-page">

    <section class="header-lined dm-checkout-page-header" aria-labelledby="dmCheckoutPageTitle1442">
        <div class="dm-checkout-page-header-text">
            <h1 id="dmCheckoutPageTitle1442">{$LANG.orderForm.checkout}</h1>
            <p>Review your account and payment details to complete your order.</p>
        </div>
    </section>
        ...
        <div class="cart-body">
            ...
            {if !$loggedin}
                <div class="already-registered clearfix">
                    <div class="pull-right float-right">
                        <button type="button" class="btn btn-info{if $custtype eq "existing"} w-hidden{/if}" id="btnAlreadyRegistered">
                            {$LANG.orderForm.alreadyRegistered}
                        </button>
                        <button type="button" class="btn btn-warning{if $custtype neq "existing"} w-hidden{/if}" id="btnNewUserSignup">
            ...
            <form method="post" action="{$smarty.server.PHP_SELF}?a=checkout" name="orderfrm" id="frmCheckout">
                ...
                {if $custtype neq "new" && $loggedin}
                    <div class="sub-heading dm-checkout-account-heading">
                    ...
                    <div id="containerExistingAccountSelect" class="row account-select-container">
                        {foreach $accounts as $account}
                            <div class="col-sm-{if $accounts->count() == 1}12{else}6{/if}">
                                <div class="account{if $selectedAccountId == $account->id} active{/if}">
                                    <label class="dm-account-choice-label" for="account{$account->id}">
                                        <span class="dm-account-choice-radio">
                                        ...
                                        <span class="address dm-account-choice-copy">
                                            <strong>
                        ...
                        <div class="col-sm-12">
                            <div class="account border-bottom{if !$selectedAccountId || !is_numeric($selectedAccountId)} active{/if}">
                                <label class="dm-account-choice-label dm-new-account-choice-label">
                                    <span class="dm-account-choice-radio">
                                    ...
                                    <span class="dm-account-choice-copy">
                                        <strong>{lang key='orderForm.createAccount'}</strong>
                                        <small>Creates a separate billing account for this order. It does not add a new credit card.</small>
                                    </span>
                                </label>
                            </div>
                        </div>
                ...
                <div id="containerExistingUserSignin"{if $loggedin || $custtype neq "existing"} class="w-hidden{/if}">
                    ...
                    {include file="orderforms/standard_cart_2/linkedaccounts.tpl" linkContext="checkout-existing"}
                </div>
                ...
                >
                    ...
                    <div{if $loggedin} class="w-hidden"{/if}>
                        {include file="orderforms/standard_cart_2/linkedaccounts.tpl" linkContext="checkout-new"}
                    </div>
                ...
                <div class="sub-heading dm-checkout-payment-heading">
                    ...
                    <div class="cc-input-container{if $selectedgatewaytype neq "CC"} w-hidden{/if}" id="creditCardInputFields">
                        {if $client}
                            <div id="existingCardsContainer" class="existing-cc-grid">
                                {include file="orderforms/standard_cart_2/includes/existing-paymethods.tpl"}
                            </div>
                        {/if}
...
{include file="orderforms/standard_cart_2/recommendations-modal.tpl"}
